<x-app-layout>
    ...
    <div class="px-4 py-6 mx-auto space-y-6 max-w-7xl">
        ...
        <div class="flex items-center justify-between">
            ...
            <a
                href="{{ route('admin.pimpinan.create') }}"
        ...
        <div class="overflow-x-auto bg-white rounded-lg shadow">
            ...
            <table class="w-full divide-y divide-gray-200 min-w-max">
                ...
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-xs font-semibold text-left text-gray-700 uppercase">
                            NIP
                        </th>
                        ...
                        <th class="px-6 py-3 text-xs font-semibold text-center text-gray-700 uppercase">
                            Aksi
                        </th>
                    </tr>
                </thead>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-3 text-sm text-center whitespace-nowrap">
                                ...
                                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-center">
                                    ...
                                    {{-- Edit --}}
                                    <a
                                        href="{{ route('admin.pimpinan.edit', $item->id) }}"
                                    ...
                                    {{-- Reset Password --}}
                                    <form
                                        action="{{ route('admin.pimpinan.reset', $item->id) }}"
                                        method="POST"
                                    >
                                        @csrf
                                    ...
                                    @if($item->is_active)
                                        ...
                                        <form
                                            action="{{ route('admin.pimpinan.destroy', $item->id) }}"
                                            method="POST"
                                            onsubmit="return confirm('Nonaktifkan pimpinan ini?')"
                                        >
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    @else
                                        ...
                                        <form
                                            action="{{ route('admin.pimpinan.activate', $item->id) }}"
                                            method="POST"
                                            onsubmit="return confirm('Aktifkan kembali pimpinan ini?')"
                                        >
                                            @csrf

                                            <button
                                                type="submit"
                                                class="w-full px-3 py-1 text-xs text-white bg-green-600 rounded hover:bg-green-700"
                                            >
                                                Aktifkan
                                            </button>
                                        </form>
