<x-app-layout>
    ...
    <div class="px-4 py-6 mx-auto space-y-6 max-w-7xl">
        ...
        <form action="{{ route('admin.dosen.sync') }}" method="POST"
            ...
            <select name="prodi_id" required class="p-2 w-full text-sm rounded-lg border sm:w-64">
        ...
        <form method="GET" class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Nama / NIDN"
                                ...
                                <div class="flex flex-col gap-2 justify-center sm:flex-row">
                                    ...
                                    @if ($d->is_active)
                                        <form method="POST" action="{{ route('admin.dosen.destroy', $d) }}">
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="px-3 py-1 w-full text-xs text-white bg-red-600 rounded hover:bg-red-700">Nonaktifkan</button>
                                        </form>
                                    @else
                                        <form method="POST" action="{{ route('admin.dosen.activate', $d->id) }}">
                                            @csrf
                                            <button type="submit"
                                                class="px-3 py-1 w-full text-xs text-white bg-green-600 rounded hover:bg-green-700">Aktifkan</button>
                                        </form>
                                    @endif
                                </div>
                            </td>
