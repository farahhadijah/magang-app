        ...
        <div class="mx-auto space-y-6 max-w-5xl">
            ...
            <div class="mb-8 text-center">
                ...
                <h1 class="text-3xl font-bold text-gray-800 sm:text-4xl">
                    Pengajuan <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-emerald-600">PKL</span>
                </h1>
            ...
            @foreach (['success', 'error'] as $msg)
                @if (session($msg))
                    <div
                        class="flex items-center gap-3 p-4 rounded-xl shadow-sm transform transition-all duration-300 animate-in slide-in-from-top-2 {{ $msg === 'success' ? 'bg-green-50 border-l-4 border-green-500 text-green-800' : 'bg-red-50 border-l-4 border-red-500 text-red-800' }}">
                        <i
                            class="fa-solid text-xl {{ $msg === 'success' ? 'fa-circle-check text-green-600' : 'fa-circle-xmark text-red-600' }}"></i>
            ...
            <form id="formPengajuan" method="POST" action="{{ route('mahasiswa.pengajuan.store') }}"
                ...
                <div class="p-6 bg-gradient-to-r to-transparent border-b border-gray-100 md:p-8 from-green-50/30">
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        <div class="md:col-span-2">
                            <label class="block mb-2 text-sm font-semibold text-gray-700">Nama Instansi <span
                ...
                <div class="p-6 md:p-8">
                    ...
                    @foreach ($dokumenFields as $name => $field)
                        <div class="mb-6 group">
                            ...
                            {{-- Upload Area --}}
                            <div
                                class="relative border-2 border-dashed border-gray-300 rounded-xl bg-gray-50/30 hover:bg-gray-50 transition-all duration-200 group-hover:border-{{ $field['color'] }}-400">
                                <input type="file" name="{{ $name }}{{ $field['multiple'] ? '[]' : '' }}"
                                    id="{{ $name }}" {{ $field['multiple'] ? 'multiple' : '' }}
                                    {{ $field['required'] ? 'required' : '' }} accept="{{ $field['accept'] }}"
                                    class="absolute inset-0 z-10 w-full h-full opacity-0 cursor-pointer">
                                <div class="p-5 text-center">
                                    <i
                                        class="fa-solid {{ $field['icon'] }} text-3xl text-gray-400 mb-2 group-hover:text-{{ $field['color'] }}-500 transition-colors"></i>
                                    <p class="text-sm text-gray-500">
                                        <span class="font-medium text-{{ $field['color'] }}-600">Klik untuk
                ...
                <div
                    ...
                    <button type="submit"
                        class="inline-flex gap-2 justify-center items-center px-6 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-green-600 to-emerald-600 rounded-xl shadow-md transition-all duration-200 hover:from-green-700 hover:to-emerald-700 hover:shadow-lg">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function setupFilePreview(inputId, listId, isMultiple = false) {
                ...
                input.addEventListener('change', function(e) {
                    ...
                    if (isMultiple) {
                        ...
                        const icon = fileType.includes('pdf') ? 'fa-file-pdf' : (fileType.includes(
                            'image') ? 'fa-file-image' : 'fa-file-alt');
                        const color = fileType.includes('pdf') ? 'red' : (fileType.includes('image') ?
                            'blue' : 'gray');
            ...
            function searchLocation(tempat) {
                ...
                fetch(url, {
                    ...
                    `;
                        ...
                        updateLocationStatus('Lokasi tidak ditemukan. Silakan cari manual melalui Google Maps.',
                            'error');
                    })
                    .finally(() => {
                        isLocating = false;
                    ...
                    fetch("{{ route('mahasiswa.pengajuan.cek-kemiripan') }}", {
                        ...
                        .then(data => {
                            if (data.mirip) {
                                warningBox.innerHTML = `
                                <i class="fa-solid fa-triangle-exclamation"></i>
                                <span> Nama ini mirip dengan <b>${data.nama_mirip}</b>. Pastikan ini memang tempat yang berbeda, jika ya abaikan pesan ini</span>
                            `;
                                warningBox.classList.remove('hidden');
                            } else {
                                warningBox.classList.add('hidden');
            ...
            btnCari.addEventListener('click', function() {
                ...
                if (!tempat || tempat.length < 3) {
                    updateLocationStatus('Masukkan nama instansi terlebih dahulu (minimal 3 karakter)',
                        'warning');
                    inputNama.focus();
                    return;
                }
            ...
            form.addEventListener("submit", function(e) {
                ...
                if (!lokasi || (!lokasi.includes("google.com/maps") && !lokasi.includes(
                    "maps.app.goo.gl"))) {
                    alert(
                        "⚠️ Lokasi harus berupa link Google Maps yang valid.\n\nContoh: https://www.google.com/maps?q=-7.123,112.456\n\nAtau klik tombol 'Buka Google Maps' untuk mencari manual.");
                    e.preventDefault();
                    return;
                }
            });
