<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-red-900">
            Histori Pengajuan Ditolak TU
        </h2>
    </x-slot>

    <div class="py-6">

        @if($pengajuansDitolak->isEmpty())
                ...
                <table class="w-full border-collapse min-w-max">
                    <thead class="bg-red-100">
                        <tr>
                            ...
                            <th class="p-3 text-left border">Catatan TU</th>
                            <th class="p-3 text-left border">Tanggal Ditolak</th>
                        </tr>
                    </thead>

                    <tbody class="bg-white">
                        @foreach ($pengajuansDitolak as $item)
                            <tr class="transition hover:bg-red-50">
                                ...
                                <td class="p-3 border">{{ $item->catatan_tu ?? '-' }}</td>
                                <td class="p-3 border">{{ $item->updated_at->format('d-m-Y') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
