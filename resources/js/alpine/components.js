...
export function registerAlpineComponents(Alpine) {
    ...
    Alpine.data("kaprodiPengajuanShow", (lokasi = null, dosenList = [], searchUrl = "/kaprodi/dosen/search") => ({
        isOpen: false,
        fileUrl: "",
        ...
        // AJAX SEARCH DOSEN
        // =====================================================
        async searchDosenAjax() {
            const keyword = this.searchDosen.trim();

            // Bersihkan hasil jika keyword kurang dari 2 karakter
            if (keyword.length < 2) {
                this.dosenSearchResults = [];
                this.dosenSearching = false;
                return;
            }

            this.dosenSearching = true;

            try {
                const response = await fetch(
                    `${this.searchUrl}?q=${encodeURIComponent(keyword)}`,
                    {
                        method: "GET",
                        headers: {
                            Accept: "application/json",
                ...
                this.dosenSearchResults = Array.isArray(data)
                    ? data
                    : [];

            } catch (error) {
                console.error("AJAX search dosen:", error);
        ...
        initMap() {
            ...
            const map = L.map("mapKaprodi").setView(
                [lat, lon],
                13
            );
